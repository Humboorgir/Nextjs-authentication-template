...
type Link = DropdownProps["items"][0];
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.ul
          ...
          className={cn(
            `absolute right-0 top-full mt-1 w-52 origin-top-right divide-y divide-foreground/10 overflow-hidden rounded-md border border-ring bg-background`,
            menuClassName
          )}>
          {items.map((item, i) => {
